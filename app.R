...
guide <- Cicerone$
  new()$ 
  step(
    el = "disease_algorithm",
    title = "Choose a type of disease and machine learning algorithms for prediciton",
    description = "The app runs different machine learning models and selects the best solution to predict Disease Prevalence given weather conditions."
  )$
  step(
    el = "plot1",
    title = "Prediction Results",
    description = "The machine learning algorithms are ranked by prediction accuracy and the first one is the most accurate algorithm"
  )$
  step(
    el = "plot2",
    title = "Which weather factors affect prediction",
    description = "Different diseates are affected by different weather factors, the weather factors are ranked by thiir importances for prediction. "
  )$
  step(
    el = "box_predict",
    title = "Custermize a Prediction yourself",
    description = "By typing in different weather condition value to check how likely the disease will outbreak"
  )
  ...
  dashboardBody(
    ...
    tabItems(
      ...
      tabItem(tabName = "dashboard",
              ...
              fluidRow(
                box(width = 12,
                    id = "disease_algorithm",
                    selectInput("select_disease", label = "Choose a disease to explore:", 
...
server <- function(input, output, session) {
  ...
  learning_result <- reactive({
    ...
    # Get all data
    dataset <- DT[Disease == input$select_disease,][, all_columns[which(!all_columns %in% c("Year","Disease","Number","Med_case","Avg_case","Med_case", "Above_Med"))], with =F]
  ...
  output$weatherfactor <- renderUI({
    ...
    lapply(factor_names, function(x){
      n <- DT[Disease == input$select_disease,][[x]]
  ...
  observeEvent(input$bt, {
    ...
    if(prediction == "No"){
      sendSweetAlert(
        session = session,
        title = paste0(input$select_disease, " is within the safe range!"),
        text = NULL,
        type = "success"
      )
    }else{
      sendSweetAlert(
        session = session,
        title = paste0(input$select_disease, " is within the danger range!"),
        text = NULL,
        type = "warning"
      )
    }
